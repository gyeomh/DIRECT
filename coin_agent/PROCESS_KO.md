...
- 총 **86개 테스트**, 전부 통과 (`pytest coin_agent/tests`).
- 순수 로직(스키마, 정규화, 비교, 선택, 예산, 상태)은 네트워크 없이 완전히 테스트됨.
- `extract()` / `adjudicate()`는 실제 vllm 서버 없이도, LLM 클라이언트를 흉내 내는(monkeypatch)
  방식으로 프롬프트 조립 → JSON 파싱 → 신뢰도 계산까지 전체 로직을 검증함.
- 통합 테스트(`questioner.py` 전체 흐름)는 위 표의 버그 1, 4번에 대한 회귀 테스트를 포함함.

---

## 14. 아직 안 한 것 / 다음 단계

- 실제 vllm/Qwen 서버를 띄워서 이 세 프롬프트를 진짜로 호출해보는 것 (지금까지는 전부
  가짜 응답으로 테스트함).
- `scripts/build_priors.py` (변별력 표 구축), `scripts/run_eval.py` (실제 평가 실행),
  `scripts/ablate.py` (단계별 성능 비교) — 구조는 다 짜여 있지만, 살아있는 모델이 있어야
  실제로 돌려볼 수 있음.
